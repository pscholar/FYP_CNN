...
def adaptive_color_threshold(flood_rgb_values):
    """
    Computes an adaptive color threshold using median absolute deviation (MAD).
    
    Parameters:
        flood_rgb_values (numpy.ndarray): Array of flood region RGB values.

    Returns:
        adaptive_threshold (float): Computed adaptive threshold.
    """
    median_rgb = np.median(flood_rgb_values, axis=0)
    mad_rgb = np.median(np.abs(flood_rgb_values - median_rgb), axis=0)
    print("Med rgb: ",mad_rgb)
    adaptive_threshold = np.linalg.norm(mad_rgb)  # Use the Euclidean norm of MAD
    print("Threshold", adaptive_threshold)
    return max(adaptive_threshold, 20)  # Ensure a minimum threshold to avoid over-segmentation
        ...
        for f_idx in floods:
            ...
            if (
                ...
                if flood_points_in_roi.size > 0:
                    ...
                    adaptive_threshold = adaptive_color_threshold(flood_rgb_values)

                    # Compute median flood color in RGB
                    median_flood_rgb = np.median(flood_rgb_values, axis=0)
                    ...
                    for y in range(image.shape[0]):
                        for x in range(image.shape[1]):
                            if roi_mask[y, x] == 1 and taxi_mask[y, x] == 0:
                                ...
                                color_distance = np.linalg.norm(pixel_rgb - median_flood_rgb)
