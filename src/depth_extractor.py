...
def extract_bounding_boxes(json_path, image_size):
    ...
    taxi_bbox = None
    flood_mask = np.zeros((image_size[0], image_size[1]), dtype=np.uint8)    
...
def get_homography_matrix(ref1,ref2,target,dx, dy):   
    ...
    for kp in keypoints2:
        ...
        translated_points.append(new_kp)
    matched_img = corr.visualize_matches(best_ref, target, keypoints1, keypoints2, good_matches)
    H, mask = corr.compute_homography(best_ref, keypoints1, translated_points, good_matches)
...
if __name__ == "__main__":
  ...
  best_ref, homography_matrix, flag,matched_img = get_homography_matrix(ref1,ref2,subimage,start_x, start_y)
  save_plot_image(MATCHES,matched_img,"Correspondences between Reference Image and Scene Image",True)
  ...
  blended_image, warped_flood_mask =  \
        warp_flood_over_reference_image(best_ref,flood_mask,
                                                        ...
                                                        alpha1=.5,alpha2=1.0)
  save_plot_image(BLENDED_IMAGE ,blended_image,"Flood Region Registered to the Reference Image",True)
  ...
  save_plot_image(TAXI_BODY,ref_taxi_body_mask,"Mask of Taxi Body from Best Matching Reference Image",True)
  ...
  save_plot_image(WARPED_FLOOD,warped_flood_mask,"Mask of Flood Region in Registered Image",True)
